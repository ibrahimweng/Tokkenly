...
for (const theme of ['dark', 'light']) {
  ...
  for (const w of WIDTHS) {
    ...
    for (const route of ROUTES) {
      ...
      if (r.over > 0) { bad++; check(`${theme} ${w} ${route} stays inside the screen`, false, `over by ${r.over}px  ${r.who}`) }
    }
  }
  await p.close()
}
check('nothing scrolls sideways, anywhere', !bad, bad ? `${bad} overflowing` : '')
await teardown(b)
